...
class Net(nn.Module):
    ...
    def forward(self, xb):
        x = F.dropout(self.network(xb), training=self.training)
        return x
...
for epoch in range(1, num_epochs + 1):
    ...
    for data, label in trainloader:
        ...
        target_one = nn.functional.one_hot(target)
        target_one_hot = torch.flatten(target_one)
        loss = criterion(output,target_one)
